<script lang="ts">
  ...
  onMount(() => {
    refresh().then(() => { error = ''; }).catch((cause) => {
      error = cause instanceof Error ? cause.message : 'Не удалось загрузить подключения';
    });
  });
...
{#if error}<p role="alert" class="mb-4 rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-800">{error}</p>{/if}
{#if notice}<p role="status" class="mb-4 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-800">{notice}</p>{/if}

<div
  class="grid items-start gap-6"
  class:lg:grid-cols-[minmax(0,1.1fr)_minmax(340px,0.9fr)]={connections.length > 0}
>
  <section class="relative overflow-hidden rounded-3xl border border-line bg-white shadow-sm" aria-labelledby="connections-title">
    <div class="flex items-start justify-between gap-4 border-b border-line px-6 py-5 sm:px-8">
      <div>
        <h2 id="connections-title" class="mt-1 text-xl font-bold tracking-tight sm:text-2xl">
          Доступные подключения
        </h2>
        <p class="mt-1 text-sm text-muted">
          Ключ можно заменить или сбросить в любой момент.
        </p>
      </div>
      <button
        type="button"
        class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-accent text-xl font-bold text-white transition hover:bg-accent-dark focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent disabled:opacity-60"
        disabled={busy || !data.form}
        onclick={() => (formOpen ? resetForm() : openCreate())}
        aria-label={formOpen ? 'Закрыть форму' : 'Добавить подключение'}
        aria-expanded={formOpen}
      >
        <span class="transition-transform" class:rotate-45={formOpen}>+</span>
      </button>
    </div>
  ...
  {#if formOpen}
    <section class="overflow-hidden rounded-3xl border border-line bg-white shadow-sm" aria-labelledby="form-title">
      <div class="border-b border-line px-6 py-5 sm:px-8">
        <p class="text-xs font-bold tracking-[0.14em] text-accent uppercase">Параметры доступа</p>
